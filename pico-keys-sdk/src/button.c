...
static bool picok_board_button_read(void) {
    ...
    return gpio_get(BUTTON_GPIO_PIN) == BUTTON_ACTIVE_LEVEL;
}
#else
...
int button_wait(void) {
    ...
    // Wait for stable button press (debounced)
    while (cancel_button == false) {
        execute_tasks();
        ...
        if (button_pressed) {
            uint32_t press_start = board_millis();
            // require the button to stay pressed for PRESENCE_DEBOUNCE_MS
            while ((board_millis() - press_start) < PRESENCE_DEBOUNCE_MS && cancel_button == false) {
                execute_tasks();
                if (board_millis() >= next_button_poll_ms) {
                    if (!picok_board_button_read()) {
                        button_pressed = false;
                        break; // unstable, go back to waiting for press
                    }
                    next_button_poll_ms = board_millis() + button_poll_interval_ms;
                }
            }
            if (button_pressed && (board_millis() - press_start) >= PRESENCE_DEBOUNCE_MS) {
                // stable press detected, now wait for release (or timeout)
