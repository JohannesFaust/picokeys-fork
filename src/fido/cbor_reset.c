...
int cbor_reset(void) {
...
#endif
    // SECURITY: require a physical button press on BUTTON_GPIO_PIN (if configured)
#if defined(PICO_PLATFORM) && defined(BUTTON_GPIO_PIN)
    printf("[SECURITY LOG] Reset Command Received via USB...\n");
    uint32_t timeout_start = board_millis();
    bool phys_pressed = false;
    while ((board_millis() - timeout_start) < 10000) {
        if (gpio_get(BUTTON_GPIO_PIN) == BUTTON_ACTIVE_LEVEL) {
            phys_pressed = true;
            break;
        }
        sleep_ms(10);
    ...
    if (!phys_pressed) {
        printf("[DEFENSE] Reset rejected: No physical button press detected!\n");
        return CTAP2_ERR_NOT_ALLOWED;
    }
#endif
